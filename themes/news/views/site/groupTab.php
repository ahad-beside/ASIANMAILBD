<div class="row">
    ...
    <div class="col-md-5">
        <?php $i=0; foreach ($data['result'] as $info): $i++;?>
            <div class="media col-md-12 mbtm_10">
                ...
                <div class="media-body">
                    ...
                    <?= Post::model()->getExcerpt($info['description'], 0, 350); ?> <a href="<?= $link ?>"><strong>বিস্তারিত...</strong></a>
                </div>
            </div>
        <?php
            if($i==2){
    ...
    <div class="col-md-7">
        ...
        <div role="tabpanel">
            ...
            <ul class="nav nav-tabs" role="tablist">
                ...
                <li role="presentation" class="<?= ($i==1)?'active':''?>"><a href="#<?= $cats->id?>" aria-controls="home" role="tab" data-toggle="tab"><?= $cats->name?></a></li>
                <?php endforeach;?>
            </ul>
            ...
            <div class="tab-content" style="padding: 10px 0px">
                <?php $i =0; foreach($subcat as $cats): $i++;?>
                <div role="tabpanel" class="tab-pane tab-group <?= ($i==1)?'active':''?>" id="<?= $cats->id?>">
                    <?php
                    //echo $cats->id;
                    $spotlight = Post::model()->getPost($cats->id, 8);
                    ...
                    <div class="media">
                        ...
                        <div class="media-body">
                            <h4 class="media-heading" id="media-heading"><strong><a href="<?= $link ?>"><?= $info['title'] ?></a></strong></h4>
                            <?= Post::model()->getExcerpt($info['description'], 0, 450); ?> <a href="<?= $link ?>"><strong>বিস্তারিত...</strong></a>
                        </div>
                    </div>
                    <?php else:?>
                        <div class="more_link"><a href="<?= Post::model()->makeLink($info['id']) ?>"><?= $info['title'] ?></a></div>
                        <?php endif;?>
                    <?php
                        endforeach;
                    }
                    ?>
                        <div class="col-md-12 allnews"><a href="<?= Category::model()->makeLink($cats->id)?>"><strong><?= $cats->name?> এর আরও খবর >></strong></a></div>
                </div>
                <?php endforeach;?>
            </div>
        </div>
