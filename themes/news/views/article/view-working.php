...
$this->pageDescription = $model->short_description;
if($model->keyword!='')
...
<div class="container">
    <div class="row margin-top">
        ...
        <div class="col-md-4">
            <?php
            $similarPost = Post::model()->getSimilarCategoryPost($model->id,5);
            if(count($similarPost)>0){
            foreach($similarPost as $post):
                ?>
                <div class="future">
                    <div class="post-heading"><span><a href="<?= $link = Post::model()->makeLink($post['id'])?>"><h4><?= $post['title']?></h4></a></span></div>
                    <div class="future-img"><a href="<?= $link?>"><?= Yii::app()->easycode->showImage($post['image'], 213, 165) ?></a></div>
