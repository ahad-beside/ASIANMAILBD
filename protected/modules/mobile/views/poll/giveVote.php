...
<div class="row">
    ...
    <div class="panel-primary">
        <div class="panel-heading page-title"><strong>অনলাইন ভোট</strong></div>
        ...
        <br>
        <a class="btn btn-xs btn-success" href="<?= $this->createUrl('//mobile/poll/viewResult', array('id'=>$poll->id,'name'=>Yii::app()->easycode->seoUrl($poll->title)))?>"><strong>ফলাফল</strong></a> &nbsp; <a class="btn btn-xs btn-info" href="<?= $this->createUrl('//mobile/poll/pastResults')?>"><strong>পুরোনো ফলাফল</strong></a>
        </div>
    </div>
    </div>
</div>
...
<script type="text/javascript">
    $('#giveVote').submit(function(){
        if($('#giveVote').find('input[type="radio"]').is(':checked')){
            ...
            $.post("<?= $this->createUrl('//mobile/poll/giveVote')?>",{optionId: optionId, pollId: pollId},function(data){
                $('.voteLoading').show();
                $('#giveVote').hide();
            }).done(function(data) {
